...
Lynx re-implements the **multi-agent architecture of CAI** on top of
...
## Design principle: opencode is the runtime, Lynx is the architecture
...
and a source of bugs. So Lynx treats opencode as the **runtime + interface**
...
2. **A plugin** (`runtime/plugin/lynx.ts`) — cross-cutting logic: the HITL
...
sandbox container
   ...
   └── ~/.config/opencode/         ← Lynx global config (baked)
        ├── opencode.json          ← permissions route intrusive tools to HITL
        ├── plugin/lynx.ts     ← HITL policy + safety floor + tools
...
A common question is whether Lynx should be _an opencode plugin_ or _a
separate agent_. The answer is **both**, and that mirrors CAI: CAI has both agent
definitions _and_ an SDK/runtime with tools and policy. In Lynx:
...
Lynx's pattern support is delivered in phases (a deliberate decision to ship
...
- **Phase 2 (implemented — essential set):** an explicit **pattern engine** in
  ...
  - `lynx_parallel` — independent tasks run concurrently, each in its own
    session/context; results aggregated.
  - `lynx_pipeline` — agents run in sequence, each fed the previous output.
  - `lynx_swarm` — peer-to-peer hand-offs; the engine routes by parsing a
...
2. **Declarative `lynx.yml`** for multi-agent line-ups (analogue of CAI's
